...
private extension CLPlayer {
    func availableDuration() -> TimeInterval? {
        ...
        let endSeconds = CMTimeGetSeconds(timeRange.duration)

        print("startSeconds=\(startSeconds),durationSeconds=\(durationSeconds),endSeconds=\(endSeconds) 总和=\(startSeconds + durationSeconds)")
...
extension CLPlayer: CLPlayerContentViewDelegate {
    ...
    func scrollTouchBegan(_ slider: CLSlider, in contentView: CLPlayerContentView) {
        sliderTimer?.suspend()
        bufferTimer?.cancel()
    }

    func scrollTouchChanged(_ slider: CLSlider, in contentView: CLPlayerContentView) {
        currentDuration = totalDuration * TimeInterval(slider.value)
        let dragedCMTime = CMTimeMake(value: Int64(ceil(currentDuration)), timescale: 1)
        player?.seek(to: dragedCMTime, toleranceBefore: .zero, toleranceAfter: .zero)
    }

    func scrollTouchEnded(_ slider: CLSlider, in contentView: CLPlayerContentView) {
        guard let playerItem = playerItem else { return }
        if slider.value == 1 {
            didPlaybackEnds()
        } else if playerItem.isPlaybackLikelyToKeepUp {
