    ...
    func panDirection(_ pan: UIPanGestureRecognizer) {
        ...
        case .began:
            if abs(veloctyPoint.x) > abs(veloctyPoint.y) {
                ...
                showToolView()
                delegate?.scrollTouchBegan(sliderView, in: self)
        ...
        case .changed:
            switch panDirection {
            case .horizontal:
//                print("changed\(abs(veloctyPoint.x))")
                cancelAutoFadeOutTooView()
                let slider = sliderView
                slider.value = slider.value + Float(veloctyPoint.x / 10000)

                delegate?.scrollTouchChanged(slider, in: self)

            case .leftVertical:
                UIScreen.main.brightness = abs(UIScreen.main.brightness - CGFloat(Float(veloctyPoint.y / 10000))) + 0.001
        ...
        case .ended, .cancelled:
            ...
            let slider = sliderView
            slider.value = slider.value + Float(veloctyPoint.x / 10000)
            delegate?.scrollTouchEnded(slider, in: self)

//            print("ended\(abs(veloctyPoint.x))")
        default: break
        }
    }
...
extension CLPlayerContentView: UIGestureRecognizerDelegate {
    func gestureRecognizer(_ gestureRecognizer: UIGestureRecognizer, shouldReceive touch: UITouch) -> Bool {
        if morePanelCollectionView.bounds.contains(touch.location(in: morePanelCollectionView)) {
            ...
            return config.isGestureInteractionEnabled
        }
        return true
    }
}
